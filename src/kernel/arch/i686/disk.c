...
bool DISK_Initialize(DISK* disk, uint8_t driveNumber)
{
    ...
    if (!i686_Disk_GetDriveParams(disk->id, &driveType, &cylinders, &sectors, &heads))
        return false;
...
bool DISK_ReadSectors(DISK* disk, uint32_t lba, uint8_t sectors, void* dataOut)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        if (i686_Disk_Read(disk->id, cylinder, sector, head, sectors, dataOut))
            return true;

        i686_Disk_Reset(disk->id);
    }
